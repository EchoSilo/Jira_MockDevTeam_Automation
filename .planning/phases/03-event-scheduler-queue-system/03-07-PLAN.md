...
<task type="auto">
  ...
  <action>
    ...
    IMPORTANT: The `action_executor` parameter is a callable that bridges TickExecutor to the existing
    CrewAI crew execution in ScenarioOrchestrator. The orchestrator will pass a method like
    `orchestrator._execute_action` which internally calls the appropriate crew (TicketLifecycleCrew,
    BlockerCrew, ReworkCrew, etc.) based on action type. This preserves existing CrewAI execution
    while adding scheduled timing.

    Integration pattern:
    - TickExecutor receives scheduled actions with `action_type` (e.g., "pick_up_task", "transition_status")
    - action_executor converts action_type to the appropriate Crew call
    - ScenarioOrchestrator._execute_action already does this mapping via its crew dict
...
- action_executor receives dict format compatible with ScenarioOrchestrator
- All tests pass
</success_criteria>
